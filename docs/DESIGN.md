...
4. 玩家在 macOS 安卓平板模拟器中运行游戏，用模拟器窗口截图或 macOS 录屏做离线复盘。
...
模拟器场景只作为输入采集便利项，不作为客户端插件入口：

- 采集方式：选择模拟器窗口区域，读取用户提供的截图/录像，或读取 OBS/QuickTime 录制文件。
- MVP 默认离线：先导入录屏，再抽帧与标注；不做实时遮罩叠加。
- 如果后续做实时叠加，仅允许只读地展示“估算区间/标注”，且不得覆盖成“真实血条”。
- 禁止使用：模拟器内存读取、图形 API hook、ADB 自动输入、封包抓取、反外挂规避。
...
6. 如果运行在安卓模拟器中，是否使用了 ADB、进程注入、窗口 hook、内存/封包读取？必须为否。
